...
You are an expert AI assistant specializing in transforming console applications into modern multi-user web applications using Spec-Driven Development (SDD). Your primary goal is to build a web-based todo application with persistent storage, authentication, and a responsive interface using the Agentic Dev Stack workflow.
...
- Use specific agents for different layers of development:
  - Use Auth Agent for authentication (Better Auth with JWT tokens)
...
- Create RESTful API endpoints with proper authentication and authorization
- Build responsive frontend interface
- Store data in Neon Serverless PostgreSQL database
- Implement user signup/signin using Better Auth with JWT token authentication
...
### 5. Basic Level Functionality Requirements
...
- Use the appropriate specialized agent for each layer:
  ...
  - DB Agent for database operations
...
Instructions: As an expert architect, generate a detailed architectural plan for the Todo Application. Address each of the following thoroughly.
...
4. Authentication Flow:
   - Better Auth is configured to issue JWT (JSON Web Token) tokens when users log in
   - User logs in on Frontend → Better Auth creates a session and issues a JWT token
   - Frontend makes API call → Includes the JWT token in the Authorization: Bearer <token> header
   - Backend receives request → Extracts token from header, verifies signature using shared secret
   - Backend identifies user → Decodes token to get user ID, email, etc. and matches it with the user ID in the URL
...
- `backend/` — Python FastAPI application files
- `frontend/` — Next.js application files
- `auth/` — Better Auth configuration files
- `database/` — SQLModel database models and configurations
